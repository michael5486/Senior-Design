...
#define bufferSize 3
#define NUM_X_REGIONS 4 //number of vertical slices containing TU, defined externally in main.cpp
...
public:
	...
	int regionReturn(myPoint point);
...
int circleBuffer::regionReturn(myPoint point) { //returns 1 if TU left frame on right side and 2 if TU left frame on left side
	int a;
	a = floor(point.getImageX() / (320 / NUM_X_REGIONS));
	return a;
}
