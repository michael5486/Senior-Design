...
void myPerson::printPerson() {
	std::printf("Printing person%d:\n", personID);
	std::printf("  Joint Locations:\n");
	std::printf("    JOINT_HEAD:           "); JOINT_HEAD.printPoint();
	std::printf("    JOINT_SHOULDER_LEFT:  "); JOINT_SHOULDER_LEFT.printPoint();
	std::printf("    JOINT_SHOULDER_RIGHT: "); JOINT_SHOULDER_RIGHT.printPoint();
	std::printf("    JOINT_SPINE_MID:      "); JOINT_SPINE_MID.printPoint();
	std::printf("    JOINT_CENTER_MASS:    "); JOINT_CENTER_MASS.printPoint();
	std::printf("  Calculated Features:\n");
	std::printf("    shoulderDistance:     %.2f\n", shoulderDistance);
	std::printf("    torsoHeight:          %.2f\n", torsoHeight);
...
double compareTorsoAndArmLengths(myPerson person1, myPerson person2) {
	...
	//std::printf("e^power: %f\n", eToPower);

	double y1 = 100 * eToPower;

	//std::printf("Arm Similarity = %f\n", y1);
	...
	//std::printf("Torso Similarity = %f\n", y2);

	//calculate average of torso and arm scores
	//std::printf("Combined Similarity: %f\n", (y1 + y2) / 2);
